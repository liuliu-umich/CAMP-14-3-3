if __name__ == "__main__":
    ...
    def main():
        ...
        input_csv_file = "Paper_code/Predictions/Swissprot/all_predictions.csv"
        dataframe = pd.read_csv(input_csv_file)

        # Ensure the 'seq' column exists
        if 'Sequence' not in dataframe.columns:
            raise ValueError("CSV file must contain a column named 'Sequence'")
        
        dataframe["sequence_final"] = dataframe['Sequence']

        # Create a new column 'deephase_score' by applying the scoring function
        dataframe['docker_score'] = dataframe['Sequence'].apply(extract_deephase_score)

        output_csv_file = "all_prediction_with_docker_deephase_score.csv"
